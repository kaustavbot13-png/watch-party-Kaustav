...
app.get('/stream', async (req, res) => {
  ...
  try {
    ...
    if (shouldRemux) {
      // If we're remuxing, we can't easily know the final content-length
      // But we can enable range requests if we're not remuxing on the fly from a stream
      // For now, let's just ensure we set a proper content type and allow the browser to stream it.
      const remuxHeaders = {
        'Content-Type': 'video/mp4',
        'Accept-Ranges': 'bytes', // Allow bytes for better seeking/duration support
        'Cache-Control': 'no-store',
        'Connection': 'keep-alive'
      };

      // If we have a content length from upstream, we can use it as a hint, 
      // though remuxing might change it slightly.
