...
async function loadVideoAsBlob(url) {
    const statusDisplay = roleStatus;
    const originalText = statusDisplay.textContent;
    
    try {
        statusDisplay.textContent = 'Loading video... 0%';
        ...
        while(true) {
            ...
            if (contentLength) {
                ...
                statusDisplay.textContent = `Loading video... ${percent}%`;
            }
        }
...
function attachPlaylistItemEvents(itemDiv) {
    ...
    runBtn.addEventListener('click', () => {
        ...
        if (url) {
            ...
            loadVideoAsBlob(streamUrl).then(blobUrl => {
...
function updatePlayerStateGuest(state) {
    ...
    // Check if URL changed - ONLY reload if URL actually changed
    ...
    if (urlChanged) {
        ...
        loadVideoAsBlob(proxyUrl).then(blobUrl => {
            videoPlayer.src = blobUrl;
            videoPlayer.load();
    ...
    // Sync playback time for guests - allow seeking to match admin's position
    // This ensures everyone watches the same part of the video
...
socket.on('play', (currentTime) => {
    if (isAdmin && Math.abs(videoPlayer.currentTime - currentTime) > 1.5) {
        ...
        // Guest: NO SEEKING - just play if paused
        isSettingState = true;
        if (videoPlayer.paused) {
            const playPromise = videoPlayer.play();
            if (playPromise !== undefined) {
...
socket.on('seek', (currentTime) => {
    if (isAdmin && Math.abs(videoPlayer.currentTime - currentTime) > 1.5) {
        ...
        videoPlayer.currentTime = currentTime;
        setTimeout(() => isSettingState = false, 100);
    } else if (!isAdmin && Math.abs(videoPlayer.currentTime - currentTime) > 1.5) {
        // Guests now respond to seek events to stay in sync with admin
        isSettingState = true;
        videoPlayer.currentTime = currentTime;
        setTimeout(() => isSettingState = false, 100);
    }
